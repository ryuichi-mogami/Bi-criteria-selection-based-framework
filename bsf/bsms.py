...
class FitnessAssignment(Survival):
    ...
    def __init__(self, roi_type, eps=10.0) -> None:
        super().__init__(filter_infeasible=True)
        self.eps = eps
    ...
    def _do(self, problem, pop, *args, n_survive=None, ideal=None, nadir=None, roi_radius=0.1, **kwargs):        
        ...
        if n_in <= n_survive:
            ...
            self.count += 1 
            self.count_each.append(self.count)                                   
            return LeastHypervolumeContributionSurvival().do(problem=problem, pop=trimmed_pop, n_survive=n_survive)
...
class BSMSEMOA(GeneticAlgorithm):
...
parse_doc_string(BSMSEMOA.__init__)
